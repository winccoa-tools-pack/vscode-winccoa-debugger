...
suite('WinCC OA Debugger — E2E multi-library breakpoints (call_multi_libs)', function () {
    ...
    suiteSetup(async function () {
        ...
        try {
            ...
            console.warn(`[multi-lib-e2e] setCurrentProject failed (non-fatal): ${(err as Error).message}`);
        }
    ...
    suiteTeardown(async function () {
        ...
        if (lifecycle.isWinccoaAvailable()) {
            await lifecycle.stop().catch((e: Error) =>
                console.error(`[multi-lib-e2e] stop failed: ${e.message}`),
            );
        }
    });
    ...
    async function continueUntilLibBp(
    ...
    ): Promise<{ threadId: number; stackFrames: Array<{ line: number; source?: { name?: string; path?: string } }> }> {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            assert.strictEqual(body?.reason, 'breakpoint', `stop must be a breakpoint (attempt ${attempt + 1})`);
            ...
            console.log(`[multi-lib-e2e] Stop at ${topSrc}:${topLine}, want ${expectedSrcSubstring}:${expectedLine} — continuing (attempt ${attempt + 1})…`);
        }
    ...
    test('BPs in math_lib and nested_lib both fire from same script', async function () {
        ...
        const mathBp = new vscode.SourceBreakpoint(
            new vscode.Location(vscode.Uri.file(mathLibPath), new vscode.Position(BP_MATH_LINE - 1, 0)),
        );
        const nestedBp = new vscode.SourceBreakpoint(
            new vscode.Location(vscode.Uri.file(nestedLibPath), new vscode.Position(BP_NESTED_LINE - 1, 0)),
        );
        // Also set a main-script BP so execution pauses predictably
        const mainBp = new vscode.SourceBreakpoint(
            new vscode.Location(vscode.Uri.file(mainScriptPath), new vscode.Position(BP_MAIN_MATH - 1, 0)),
        );
        ...
        try {
            ...
            await helper.startSession(
                undefined,
                buildLaunchConfig('E2E: multi-lib BPs'),
                25_000,
            );
            ...
            assert.strictEqual(entrySt.stackFrames[0].line, STOP_LINE,
            ...
            const mathResult = await continueUntilLibBp(
                helper, entryThreadId, BP_MATH_LINE, 'math_lib',
            );
            console.log('[multi-lib-e2e] math_lib BP confirmed ✔');

            // ── Hit nested_lib BP ─────────────────────────────────────────────
            const nestedResult = await continueUntilLibBp(
                helper, mathResult.threadId, BP_NESTED_LINE, 'nested_lib',
            );
    ...
    test.skip('BP in depth-2 lib (debugger_lib via nested_lib) fires with 3-frame stack', async function () {
        ...
        const deepBp = new vscode.SourceBreakpoint(
            new vscode.Location(vscode.Uri.file(deepLibPath), new vscode.Position(BP_DEEP_LINE - 1, 0)),
        );
        // Main-script BP to ensure execution control
        const mainBp = new vscode.SourceBreakpoint(
            new vscode.Location(vscode.Uri.file(mainScriptPath), new vscode.Position(BP_MAIN_NESTED - 1, 0)),
        );
        ...
        try {
            ...
            const deepResult = await continueUntilLibBp(
                helper, entryThreadId, BP_DEEP_LINE, 'debugger_lib',
            );
            ...
            assert.ok(frame0Src.toLowerCase().includes('debugger_lib'),
                `frame 0 must be debugger_lib, got "${frame0Src}"`);
            assert.ok(frame1Src.toLowerCase().includes('nested_lib'),
                `frame 1 must be nested_lib, got "${frame1Src}"`);
            assert.ok(frame2Src.toLowerCase().includes('call_multi_libs'),
